...
.. Note:: Начиная с версии YaPB 2.10, был добавлен новый формат вэйпоинтов названный **Graph**. Благодаря этому, ограничение точек было повышено до 2048, направления кемперских точек теперь могут настраиватся вверх и вниз, а не только влево и вправо и понижен размер вэйпоинт файлов. YaPB также продолжает поддерживать старый **PWF** формат. Вэйпоинты (разноцветные полосы) теперь называются **Точки** начиная с версии YaPB 4.x. Редактор вэйпоинтов также был переименован в редактор графов.


Как выгядят вэйпоинты в игре?
----------------------------------------
...
================================
Использование ``yb g delete`` удалит вэйпоинт ближайший к игроку. Вэйпоинт ДОЛЖЕН быть в пределах 55 юнитов от игрока (примерно 1/2 роста игрока), чтобы его можно было удалить. Вам нужно будет встать достаточно близко к вэйпоинту, чтобы его можно было удалить. Это предотвратит случайное удаление вэйпоинта на другой стороне комнаты. При удалении вэйпоинта вы услышите звук, указывающий на то, что вэйпоинт был удалён (тот же звук, который издает мина, когда ставится на стену).
...
Каждый раз, когда вы приближаетесь к вэйпоинту, желтые или зелёные линии будут нарисованы ко всем другим вэйпоинтам, которые бот будет считать "достижимыми". Если соединение является двухсторонним, линия отображается желтым цветом, а односторонние соединения отображаются зелёным цветом. Эти "достижимые" вэйпоинты будут вэйпоинтами, которые четко видны из текущего местоположения. Определённые вэйпоинты будут запрещены как достижимые по той или иной причине. Например, вэйпоинты, подвешенные в воздухе над ботом, не будут считаться достижимыми, поскольку бот не может прыгнуть достаточно высоко, чтобы добраться до них. Также не будут считаться достижимыми вэйпоинты, расположенные слишком далеко от текущего местоположения. У вас могут быть вэйпоинты, расположенные достаточно близко друг к другу, но с широким промежутком, который будет слишком широк для прыжка. Если дальний вэйпоинт находится достаточно близко и четко виден, он всё равно будет отображаться как "достижимый", поскольку в настоящее время у нас нет метода, чтобы определить, может ли бот добраться до этого вэйпоинта или нет.
...
.. note:: На Es_ картах, :purple:`целевые` вэйпоинты отмечают зону побега для Террористов. Вы можете следовать тем же правилам что и для As_ карт. Теперь вы можете задаться вопросом, как определить точную функцию :purple:`целевого` вэйпоинта? Не волнуйтесь, это полностью зависит от карты, вам не нужно ничего с этим делать. Все боты обеих участвующих команд автоматически знают, какая :purple:`цель карты` на данный момент, им нужна только точка которая направляет их туда. Вы можете добавить этот тип вэйпоинта, выбрав ``8. Целевой`` из меню "\ :green:`Тип вэйпоинта`\ ". :purple:`Цвет целевых вэйпоинтов фиолетовый`, как вы можете видеть на картинках ниже.
...
На рисунке ниже показаны те же два вэйпоинта с входящим соединением (от левого к правому вэйпоинту), :teal:`Входящее одностороннее соединение отображается бирюзовым`, как вы можете видеть на картинке ниже.
...
.. note:: Если вы создали :green:`исходящее путевое соединение` от вэйпоинта A к вэйпоинту B, оно будет отображаться зелёным. А когда вы подойдёте к вэйпоинту B, путевое соединение станет бирюзовым как :teal:`входящее путевое соединение`.
...
.. figure:: images/deleted_outgoing_path.png
    ...
    Удалено :green:`исходящее путевое соединение`, осталось только :teal:`входящее путевое соединение` (от выбранного вэйпоинта до вэйпоинта на котором вы стоите)

Снова выберите ``4. Удалить путь`` чтобы удалить :teal:`входящее путевое соединение`.
...
#. Выберите ``3. Бинаправленный (Оба пути)`` чтобы создать двунаправленное (двухстороннее) путевое соединение между вэйпоинтом на котором вы стоите и выбранном вэйпоинтом.


Флаги вэйпоинтов
=========================

YaPB имеет 5 флагов для вэйпоинтов, которые из них:
